  ...
  return (
    <div>
      <p className="text-[12px] xs:text-[14px] max-lg:text-center sm:text-lg font-normal text-soft-dark">
        Je suis toujours ouvert à discuter d'une offre de stage ou d'une
      ...
      <div className="mx-2">
        <form className="flex flex-col gap-4 mt-4" onSubmit={handleSubmit}>
          ...
          <button
            ...
            className="btn gap-3 max-lg:mx-auto btn-primary rounded-sm mt-5 text-[13px] md:text-[16px] w-fit font-semibold lg:mt-12.5 p-2 md:px-4 disabled:opacity-60"
          >
            {status === "sending" ? "Envoi en cours..." : "Envoyer"}
            {status !== "sending" && telegramSVG}
          </button>

          {status === "success" && (
            <p className="text-green-600 text-[13px] md:text-[15px] font-medium">
              Message envoyé avec succès ! Je vous répondrai rapidement.
            </p>
          )}
          {status === "error" && (
            <p className="text-red-500 text-[13px] md:text-[15px] font-medium">
              Une erreur est survenue. Vous pouvez aussi m'écrire directement à{" "}
              <a
                href="mailto:tinafaniry0802@gmail.com"
                className="underline"
              >
                tinafaniry0802@gmail.com
              </a>
              .
            </p>
          )}
